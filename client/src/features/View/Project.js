    ...
    projectContract.methods
      ...
      .then((res) => {
        const newTotal = parseInt(
          res.events.FundReceived.returnValues.currentTotal,
          10
        );
  ...
  return (
    <div className="project-card">
      ...
      <div className={`fund-options ${funding !== 0 ? "flex" : ""}`}>
        ...
        {funding !== 0 && (
          <button onClick={() => setROpen(true)}>Refund</button>
        )}
      </div>
      ...
      {rOpen && (
        <RefundModal setROpen={setROpen} project={project} refund={refund} />
      )}
      <p>
        <a
          className="link"
